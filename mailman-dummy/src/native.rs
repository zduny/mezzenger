...
impl<Message> mailman::Reliable for Transport<Message> {}

impl<Message> mailman::Order for Transport<Message> {}

impl<Message> mailman::Close for Transport<Message> {
...
impl<Message> mailman::Send<Message> for Transport<Message>
where
    Message: Clone,
{
...
impl<'a, Message> Future for Send<'a, Message> {
    type Output = Result<(), mailman::Error<Infallible>>;
    ...
    fn poll(mut self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        if self.message.is_some() {
            ...
            if self.transport.is_closed() {
                self.message.take();
                Poll::Ready(Err(mailman::Error::Closed))
            } else {
                self.transport
                    .state
                    .lock()
...
impl<Message> mailman::Receive<Message> for Transport<Message> {
    type Error = Infallible;
...
///
/// [receive]: mailman::Receive::receive
...
impl<'a, Message> Future for Receive<'a, Message> {
    type Output = Result<Message, mailman::Error<Infallible>>;
